...
same_identity_rule = IF(OR("female (?x)", "male (?x)"), THEN("same-identity (?x) (?x)"))
sister_rule = IF(AND("parent (?x) (?y)", "daughter (?z) (?x)", NOT("same-identity (?z) (?y)")), THEN("sister (?z) (?y)"))
brother_rule = IF(AND("parent (?x) (?y)", "son (?z) (?x)", NOT("same-identity (?z) (?y)")), THEN("brother (?z) (?y)"))
sibling_rule = IF(OR("sister (?x) (?y)", "brother (?x) (?y)"), THEN("sibling (?x) (?y)"))
grandparent_rule = IF(AND("parent (?x) (?y)", "son (?x) (?z)", NOT("same-identity (?z) (?y)")), THEN("grandparent (?z) (?y)"))
cousin_rule = IF(AND("parent (?x) (?y)", "parent (?z) (?i)", OR("sibling (?x) (?z)", "sibling (?z) (?x)"), NOT("same-identity (?i) (?y)")), THEN("cousin (?i) (?y)"))
father_rule = IF(AND("parent (?x) (?y)", "male (?x)"), THEN("father (?x) (?y)"))
mother_rule = IF(AND("parent (?x) (?y)", "female (?x)"), THEN("mother (?x) (?y)"))

family_rules = [same_identity_rule, daughter_rule, son_rule,
                sister_rule, brother_rule, sibling_rule,
                grandparent_rule, cousin_rule,
                father_rule, mother_rule]                    # fill me in
...
HOW_MANY_HOURS_THIS_PSET_TOOK = 'forever'
WHAT_I_FOUND_INTERESTING = 'chess lecture'
WHAT_I_FOUND_BORING = 'parent rules'
